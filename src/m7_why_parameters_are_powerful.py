...
def better_draw_circles(radiiInc):
    window = rg.RoseWindow(400, 400)

    center = rg.Point(200, 200)
    for k in range(21):
        circle = rg.Circle(center, radiiInc * k)
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
...
def run_test_even_better_draw_circles():
    ...
    print('Testing  draw_circles:  See graphics window')
    print('--------------------------------------------------')
    even_better_draw_circles(3, 29, rg.Point(100, 100), "blue")


def even_better_draw_circles(radiiInc, circleNumb, centerLoc, circleColor):
    window = rg.RoseWindow(400, 400)

    center = centerLoc
    for k in range(circleNumb+1):
        circle = rg.Circle(center, radiiInc * k)
        circle.outline_color = str(circleColor)
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
